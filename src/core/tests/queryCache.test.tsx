...
describe('queryCache', () => {
  ...
  describe('findAll', () => {
    test('should filter correctly', async () => {
      ...
      expect(queryCache.findAll({})).toEqual([query1, query2, query3, query4])
      expect(queryCache.findAll({ queryKey: key1, active: false })).toEqual([
        query1,
      ])
      expect(queryCache.findAll({ queryKey: key1, active: true })).toEqual([])
      expect(queryCache.findAll({ queryKey: key1, stale: true })).toEqual([])
      expect(queryCache.findAll({ queryKey: key1, stale: false })).toEqual([
        query1,
      ])
      expect(
        queryCache.findAll({ queryKey: key1, stale: false, active: true })
      ).toEqual([])
      expect(
        queryCache.findAll({ queryKey: key1, stale: false, active: false })
      ).toEqual([query1])
      expect(
        queryCache.findAll({
          queryKey: key1,
      ...
      expect(
        queryCache.findAll({ queryKey: [{ a: 'a' }], active: true })
      ).toEqual([])
      expect(
        queryCache.findAll({ queryKey: [{ a: 'a' }], inactive: true })
      ).toEqual([query3])
      expect(
